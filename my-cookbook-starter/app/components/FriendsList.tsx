...
export default function FriendsList() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      if (!ignore) setMe(user.id);

      // ---------- Incoming Requests (pending where addressee_id = me) ----------
      setLoadingReq(true);
      {
        ...
        if (reqErr) {
          ...
          const requesterIds: string[] = (reqRows ?? []).map(r => String(r.requester_id));
      ...
      // ---------- Outgoing pending requests (for "Requested" state) ----------
      {
      ...
      setLoadingFriends(true);
      {
        ...
        if (accErr) {
          ...
          const otherIds = (accRows ?? []).map(r => {
            const req = String(r.requester_id);
            const add = String(r.addressee_id);
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => acceptRequest(p.id)} style={btnGreen} aria-label="Accept request">
                        Accept
                      </button>
                      <button onClick={() => declineRequest(p.id)} style={btnGray} aria-label="Decline request">
                        Decline
                      </button>
                    </div>
                  </li>
  ...
  }, [loadingReq, requests, loadingFriends, friends, acceptedIds, requestedOut]);
